...
func StaffRegister(w http.ResponseWriter, r *http.Request) {
	...
	staff := model.User{
		...
		Right:        Right,
		Status:       Status,
		RegisterTime: time.Unix(JoinTime, 0),
	}
...
func StaffMessageModify(w http.ResponseWriter, r *http.Request) {
	...
	staff.ID, _ = strconv.Atoi(r.PostForm["ID"][0])
	staff.Name = r.PostForm["Name"][0]
	staff.HeadImg = r.PostForm["HeadImg"][0]
	staff.Sex = r.PostForm["Sex"][0]
	staff.Phone = r.PostForm["Phone"][0]
	staff.Password = r.PostForm["Password"][0]
	staff.Right, _ = strconv.Atoi(r.PostForm["Right"][0])
	var RTime int64
	RTime, _ = strconv.ParseInt(r.PostForm["RegisterTime"][0], 10, 64)
	staff.JoinTime = time.Unix(RTime, 0)
	//staff.Status, _ = strconv.Atoi(r.PostForm["Status"][0])
